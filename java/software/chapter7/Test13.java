...
public class Test13 {
	...
	static int kopo41_sumEng=0; // 영어 접수 누적 총합
	static int kopo41_sumMat=0; // 수학 점수 누적 총합
	static int kopo41_sumsum=0; // 총합 점수 누적 총합
	static int kopo41_sumAvg=0; // 평균 점수 누적 총합
	
	static float kopo41_avgkor=0; //누적 국어점수 평균
	static float kopo41_avgeng=0; //누적 영어점수 평균
	static float kopo41_avgmat=0; //누적 수학점수 평균
	static float kopo41_avgsum=0; //누적 총점의 평균
	static float kopo41_avgavg=0;  //누적 평균점수의 평균
	
	static int kopo41_sumKor_page; //현재 페이지 국어점수 총합
	static int kopo41_sumEng_page; //현재 페이지 영어점수 총합
	static int kopo41_sumMat_page; //현재 페이지 수학점수 총합
	static int kopo41_sumSum_page; //현재 페이지 총점의 총합
	static float kopo41_sumAvg_page; //현재 페이지 평균점수의 총합
	
	static float kopo41_avgkor_page; //현재 페이지 국어점수 평균
	static float kopo41_avgeng_page; //현재 페이지 영어점수 평균
	static float kopo41_avgmat_page; //현재 페이지 수학점수 평균
	static float kopo41_avgsum_page; //현재 페이지 총점의 평균
	static float kopo41_avgavg_page;  //현재 페이지 평균점수의 평균
	...
	public static void kopo41_page_info(int kopo41_start, int kopo41_finish) { 
		...
		for(int kopo41_j=kopo41_start; kopo41_j<=kopo41_finish; kopo41_j++ ) {
			...
			kopo41_sumAvg_page+=kopo41_rec1.kopo41_avg(); //현재 페이지 평균점수의 총합
		}
