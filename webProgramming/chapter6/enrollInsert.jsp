...
<%
	...
	ResultSet rset = stmt.executeQuery("select max(id) from hubo"); 
	...
	stmt.execute("insert into hubo (id, name) values ("+Integer.toString(NewstdId)+", '"+ cTmpHan+"');");
	
	for (int i=1; i<=9; i++){
		stmt.execute("insert into tupyo (id, age) values ("+NewstdId+", '"+ i+"');");
	}
