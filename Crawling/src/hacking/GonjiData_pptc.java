...
public class GonjiData_pptc {
	public static void main(String[] args) throws IOException{
		try {
			GonjiData_pptc gonjiData_pptc = new GonjiData_pptc();
	...
	public GonjiData_pptc() throws InterruptedException, IOException {
		super();
		...
		base_url = "https://www.kopo.ac.kr/pptc/index.do";
		driver.get(base_url);
		...
		loop: while (true) {
			try {
				for (int p = 2; p <= 6; p++) {
					...
					driver.findElement(By.xpath("/html/body/div[2]/article/div[1]/section[2]/div/div[4]/div[2]/a["+p+"]")).click();
					if (page == 2 && p == 3) {
						driver.findElement(
					...
					try {
						for (int i = 1; i <= 10; i++) {
							...
							str = "석유화학공정기술교육원" + "," + title + "," + writer + "," + date + "," + url + "\n";
